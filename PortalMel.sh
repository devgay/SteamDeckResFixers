...
export FILE="/run/media/deck/SteamDeckSD/steamapps/common/Portal Stories Mel/portal_stories/cfg/video.txt"
...
sed -i "s/\"setting.defaultres\"		.*/\"setting.defaultres\"		\"$DECK_SCR_WIDTH\"/" "$FILE"
sed -i "s/\"setting.defaultresheight\"		.*/\"setting.defaultresheight\"		\"$DECK_SCR_HEIGHT\"/" "$FILE"
